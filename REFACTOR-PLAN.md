...
- 管理员密码使用 `cmd/vmqfox-password` 生成 bcrypt 哈希后写入。
...
- [ ] 对照当前前端 API 封装、Android 请求代码和商户接口，形成最终支持路径清单。
- [ ] 将 `router.go` 中每条 501 路由标记为“实现”或“删除”。
- [ ] 固化每条保留接口的 HTTP 方法、鉴权要求、输入来源、签名和响应格式。
- [ ] 确认 PNG、HTML、Layui 和 JSON envelope 等特殊响应是否仍有调用方。
- [ ] 固定当前 Go 重构基线，后续按阶段拆分变更。
...
- [ ] 实现单管理员信息和列表查询，不新增用户领域模型。
- [ ] 实现静态菜单响应，不引入菜单仓储。
- [ ] 在订单 usecase 中补齐按 ID 删除、过期处理和最后订单清理。
- [ ] 将补单定义为人工确认到账业务：校验可操作状态，在事务内更新付款状态和支付时间、释放价格锁并写入通知 outbox；handler 不直接调用商户 URL。
- [ ] 复用设置服务实现监控配置更新。
- [ ] 根据阶段 0 结论实现或删除 `/api/config/save`。
- [ ] 为所有破坏性管理操作统一配置 Token 鉴权和写模式保护。
...
- [ ] 核对“创建订单 → 分配价格锁 → 选择二维码 → 查询支付页”的完整流程。
- [ ] 核对“监控推送 → 支付事件幂等 → 订单状态更新 → 释放价格锁 → 写入 outbox”的事务流程。
- [ ] 核对“通知投递 → 新版 POST → 历史 GET 回退 → 重试/完成”的异步流程。
- [ ] 对齐新旧订单创建签名、返回 URL 签名和通知签名。
- [ ] 对齐 query、form、JSON 中数字字符串和空值的规范化行为。
- [ ] 保持二维码 PNG 与 `isHtml=1` 跳转页面的正确 Content-Type。
- [ ] 删除阶段 0 已确认废弃的历史路由，不保留长期 501。
...
- [ ] 将 `database/schema.sql` 固定为唯一数据库初始化文件。
- [ ] 校验 `settings`、`admin_credentials`、`qrcodes`、`orders`、`price_locks`、`payment_events`、`notification_outbox` 七张表。
- [ ] 校验唯一约束、事务所需索引、字符集、时间精度和金额字段。
- [ ] 明确空库初始化后必须设置的管理员、通讯密钥、回调地址和收款配置。
- [ ] 使用 `cmd/vmqfox-password` 生成管理员 bcrypt 哈希，并提供最小初始化步骤。
- [ ] 确认缺少管理员或业务密钥时接口返回明确错误，不写入无效订单。
...
- [ ] 收敛 `Dockerfile`，运行镜像只包含 Go 服务及必要系统依赖。
- [ ] 收敛 `docker-compose.yml`，默认以 `VMQ_RUNTIME_MODE=writer` 启动单个 API 实例。
- [ ] 收敛 Nginx 配置，将正式 API、监控和兼容路径转发到 Go HTTP 服务。
- [ ] 校验 `/health` 与 `/ready` 在容器和网关中的使用方式。
- [ ] 收敛 `entrypoint.sh` 和 `env.example`，移除 PHP、Session、Redis 等失效变量。
- [ ] 收敛 GitHub Actions，只发布 Go 二进制、Schema、示例配置和多架构镜像。
- [ ] 明确通知 worker 与生命周期 worker 在 writer 模式下随应用启动。
...
- [ ] 删除 `app/`、根目录 PHP `config/`、`route/`、PHP `public/` 和 `vendor/`。
- [ ] 删除 Composer 文件、ThinkPHP 入口、旧构建脚本和 `vmq.sql`。
- [ ] 删除 PHP-FPM、旧 Nginx rewrite、Travis PHP 构建及失效部署文件。
- [ ] 保留仍承担外部协议适配职责的 Go 兼容代码；不因删除 PHP 源码而破坏已确认的调用契约。
